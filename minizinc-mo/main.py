...
def build_solver(instance, config, statistics):
  ...
  if config.algorithm == "osolve-mo":
    solver = osolve_mo
  elif config.algorithm == "osolve-mo-then-uf":
...
def build_osolver(instance, config, statistics):
  ...
  return OSolve(instance, statistics, Timer(config.timeout_sec), config.cores, free_search, config.fzn_optimisation_level)
